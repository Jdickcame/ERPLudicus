...
// ... (Interfaces Product, Customer, CartItem iguales) ...
interface Product {
  id: number;
  name: string;
  sku: string;
...
const NewSale = () => {
  ...
  const handleProcessSale = async (paymentData: any) => {
    ...
    try {
      ...
      const saleData = response.data; // Aquí viene el ID

      setLastSaleId(saleData.id);

      // 2. Limpiamos el carrito y datos
      setCart([]);
      setSelectedCustomerId(null);
      loadData();

      // 3. EN LUGAR DE ABRIR PDF, MOSTRAMOS EL MODAL DE ÉXITO 🎉
  ...
  const printTicket = async () => {
    ...
    try {
      ...
      // 2. Creamos la URL del Blob
      const pdfBlob = new Blob([response.data], { type: "application/pdf" });
      const pdfUrl = window.URL.createObjectURL(pdfBlob);

      // --- TRUCO: IFRAME INVISIBLE ---
      // Creamos un elemento iframe en memoria
      const iframe = document.createElement("iframe");

      // Lo ocultamos visualmente (pero debe existir en el DOM para funcionar)
      ...
      // Esperamos a que el PDF cargue completamente en el iframe
      iframe.onload = () => {
        if (iframe.contentWindow) {
          // Lanzamos la orden de imprimir
          iframe.contentWindow.focus(); // Asegura el foco
          iframe.contentWindow.print();
        }
      };

      // (Opcional) Limpieza: Eliminar el iframe después de un tiempo para no llenar la memoria
      // Le damos 1 minuto para que el usuario imprima tranquilamente
      setTimeout(() => {
  ...
  const searchCustomer = async (docNumber: string) => {
    ...
    setIsLoading(true);
    try {
      // Llamamos al endpoint nuevo que creamos
      const res = await api.get(
        `/sales/customers/search_doc/?doc=${docNumber}`,
      );

      // Si el backend responde, es porque lo encontró o lo creó
      const customerData = res.data;

      // Actualizamos la lista local (para que React lo detecte)
      setCustomers((prev) => {
        // Filtramos para quitar al cliente si ya existía (para poner la versión nueva)
        const filtered = prev.filter((c) => c.id !== customerData.id);
        // Agregamos la versión fresca al final
  ...
  return (
    <div className="h-[calc(100vh-80px)] flex flex-col md:flex-row gap-4 p-4 bg-slate-100 overflow-hidden font-sans">
      {/* --- AQUÍ VA EL MODAL --- */}
      {isPaymentModalOpen && (
        <PaymentModal
          total={totalToPay}
      ...
      {/* --- MODAL DE ÉXITO CON BOTÓN DE IMPRIMIR --- */}
      {showSuccessModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm animate-in fade-in duration-200">
          <div className="bg-white p-8 rounded-2xl shadow-2xl w-96 text-center transform scale-100 animate-in zoom-in-95 duration-200">
            ...
            <div className="flex flex-col gap-3">
              {/* BOTÓN GIGANTE DE IMPRIMIR */}
              <button
                type="button" // 👈 ¡AGREGA ESTO! OBLIGATORIO
                onClick={printTicket}
      ...
      <div className="w-full md:w-[420px] flex flex-col bg-white rounded-xl shadow-xl border border-slate-200 h-full relative">
        <div className="p-4 border-b border-slate-100 bg-slate-50 rounded-t-xl">
          <div className="flex justify-between items-center mb-3">
            ...
            {/* Indicador Visual del Documento */}
            <span
          ...
          {/* 👇 AQUÍ ESTÁ EL NUEVO BUSCADOR INTELIGENTE 👇 */}
        ...
        <div className="p-5 bg-white border-t border-slate-200 shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.05)] z-20">
          ...
          <div className="flex gap-2 h-14">
            {/* 👇 BOTÓN GIGANTE QUE ABRE EL MODAL */}
            <button
