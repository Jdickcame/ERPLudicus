...
const EventCreatePage = () => {
  ...
  // 👇 Estados Modo Avanzado (Perfiles de Formularios por Categoría) 👇
  ...
  // 👇 Funciones Modo Avanzado (Perfiles) 👇
  const addProfile = () => {
    setFormProfiles([
      ...formProfiles,
      {
  ...
  // 👇 Funciones Modo Avanzado (Preguntas dentro de un Perfil) 👇
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    // Validaciones fuertes para los Perfiles
    if (isAdvanced) {
    ...
    try {
      const payload = {
        ...
        // Guardamos el array de perfiles completo en la base de datos
      ...
      toast.error(error.response?.data?.error || "Error al guardar el evento.");
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <div className="p-6 max-w-5xl mx-auto animate-in fade-in duration-500 mb-20">
      <div className="flex items-center gap-4 mb-8">
        ...
        <div>
          ...
          <p className="text-slate-500 font-medium">
            Sede activa:{" "}
            <strong className="text-blue-600">{currentBranch?.name}</strong>
          </p>
        </div>
      </div>
      ...
      <form onSubmit={handleSave} className="space-y-6">
        ...
        {/* 👇 BLOQUE 3: MODO AVANZADO (PERFILES DE FORMULARIOS) 👇 */}
        <div className="bg-white p-8 rounded-2xl shadow-sm border border-slate-200">
          <label className="flex items-center gap-3 cursor-pointer mb-4">
            <input
              type="checkbox"
            ...
            <div className="bg-indigo-50/50 border border-indigo-100 p-5 rounded-xl animate-in slide-in-from-top-2 space-y-6">
              {formProfiles.length === 0 ? (
                <div className="text-center p-8 border-2 border-dashed border-indigo-200 rounded-xl text-indigo-500 text-sm font-medium bg-white">
                  ...
                  <br />
                  Ejemplo: Un perfil para "Adultos" y otro para "Niños".
                </div>
              ) : (
              ...
              >
                <LayoutList size={18} /> AÑADIR NUEVO PERFIL DE FORMULARIO
              </button>
            </div>
          )}
        </div>
        ...
        <div className="pt-4 flex justify-end">
          <button
            type="submit"
            disabled={saving}
            className="bg-blue-600 hover:bg-blue-700 text-white px-8 py-4 rounded-xl font-black tracking-wider transition disabled:opacity-50 shadow-md flex items-center gap-2 text-lg"
          >
            {saving ? (
